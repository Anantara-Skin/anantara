...
async function getCart(token, env) {
  const raw = await env.CART_KV.get(cartKey(token));
  if (!raw) return { token, items: [], updatedAt: Date.now() };
  return JSON.parse(raw);
}

async function saveCart(cart, env) {
  await env.CART_KV.put(cartKey(cart.token), JSON.stringify(cart), {
...
export default {
  async fetch(request, env, ctx) {
    ...
    let token =
      ...
      parseCookie(request.headers.get("Cookie") || "", "cart_token");
    ...
    if (url.pathname === "/cart/add" && request.method === "POST") {
      ...
      const response = jsonResponse({ success: true, cart }, 200, origin);
      response.headers.append(
        "Set-Cookie",
        `cart_token=${token}; Path=/; Max-Age=${CART_TTL_SECONDS}; SameSite=None; Secure`
      );
      return response;
    }
    ...
    if (url.pathname === "/cart" && request.method === "DELETE") {
      if (token) await env.CART_KV.delete(cartKey(token));
