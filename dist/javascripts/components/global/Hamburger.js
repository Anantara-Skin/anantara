...
class m extends (r(e).with(l)) {
    ...
    initCache() {
        ...
        "left" === this.options.contentPosition ? (this.animationOpen = this.classes.openAnimationLeft,
        this.animationClose = this.classes.closeAnimationLeft) : (this.animationOpen = this.classes.openAnimationRight,
        this.animationClose = this.classes.closeAnimationRight),
        this.selectors.button = this.element.querySelector("[data-js-hamburger-button]"),
        this.selectors.content = this.element.querySelector("[data-js-hamburger-content]"),
        this.selectors.navigationAccordion = this.element.querySelectorAll("[data-js-navigation-accordion]"),
        this.selectors.headerAreaWrapper = this.element.closest("[data-js-header-area-wrapper]"),
        this.selectors.headerMobileNavigation = this.element.closest("[data-js-header-mobile-navigation]")
    }
    initState() {
        this.state.active = !1,
    ...
    bindEvents() {
        ...
        this.options.expandActiveAccordionItem && s("accordion.initialised", this.element, this.onAccordionInitialised.bind(this))
    }
    ...
    expandActiveAccordionItem(e) {
        if (!this.options.expandActiveAccordionItem)
            return;
        const t = (e = e || this.element).querySelectorAll("[data-js-accordion-item] a.c-navigation__link")
    ...
    onKeyboardControl(e) {
        ...
        case " ":
            ...
            break;
        case "Escape":
            this.element.classList.contains(this.classes.menuSubmenuActive) ? t.emit("hamburgermenu.close") : this.close(),
            this.selectors.button.focus()
        }
    }
    onTabKeyDown(e) {
        this.state.active && (e.shiftKey || "last" !== e.target.getAttribute("data-js-focus-order") || (e.preventDefault(),
        this.selectors.button.focus()),
        e.shiftKey && e.target === this.selectors.button && (e.preventDefault(),
    ...
    open() {
        if (t.emit("global.close"),
        this.state.accordionItemsToExpand && this.state.accordionItemsToExpand.forEach(e => {
            this.openAccordionItem(e)
        }
        ),
        ...
        this.selectors.headerMobileNavigation && this.selectors.headerMobileNavigation.classList.add(this.classes.hamburgerOpened),
        this.selectors.headerAreaWrapper) {
            ...
            this.state.headerOffsetPosition !== t && (this.state.headerOffsetPosition = t,
            this.selectors.content.style["bottom" === this.options.headerPosition ? "bottom" : "top"] = `${this.state.headerOffsetPosition}px`)
        }
        n(this.animationOpen, this.selectors.content).then( () => {
            this.setFocusedOrder(this.element),
            this.state.isKeyboardControl && (this.selectors.button.blur(),
            setTimeout( () => {
    ...
    close(e=!1) {
        ...
        this.selectors.navigationAccordion && this.selectors.navigationAccordion.forEach(e => {
            o("accordion.close", e)
        }
        ),
        this.selectors.headerMobileNavigation && this.selectors.headerMobileNavigation.classList.remove(this.classes.hamburgerOpened),
    ...
    toggleSubMenu(e) {
        e ? (this.resetFocusedOrder(this.element),
        this.state.contentScrollPosition = this.selectors.content.scrollTop,
        this.selectors.content.scrollTop = 0,
        this.element.classList.add(this.classes.menuSubmenuActive)) : (this.selectors.content.scrollTop = this.state.contentScrollPosition,
    ...
    destroy() {
        ...
        t.removeListener("registration.success", this.loginRegistrationSuccess, this),
